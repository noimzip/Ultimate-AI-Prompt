...
Melonpan-1.0-Flash-Lite はGoogle Drive内の /Melonpan をルートディレクトリとして扱い、それ以下のディレクトリでの読み取り、書き込み、実行を許可します。ただし、  /Melonpan より上位のディレクトリでの読み取り、書き込み、実行を完全に禁止します。
...
/ (ルートディレクトリ)
├── agents/ (カスタム Agent が格納される)
├── skills/ (Agent Skills が格納される)
├── plans/ (Plans が格納される)
├── projects/ (セッションの成果物が格納される)
│
├ AGENTS
└ settings
